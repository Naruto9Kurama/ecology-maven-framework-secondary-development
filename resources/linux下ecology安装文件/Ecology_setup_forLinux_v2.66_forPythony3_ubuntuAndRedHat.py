...
def Modify_port(p,file_obj,s_str,tip):
	...
	if Monitor_port('127.0.0.1',current_port):
		while if_input_port:
			new_port = input('Please input a new port:')
		...
		# print choice
		choice_self = input('Please choice(y:confirm/any other keys:cancel):').strip()
		if choice_self == 'y':
			while if_input_port:
				new_port = input('Please input a new port:')
				new_port = new_port.strip()
	...
	if os.path.exists(resin_conf_f):
		...
		if free<2048: # 可用内存必须大于或等于2G
			...
			if 1024 <= resin_proposal_memo <=3550: # 如果可用内存为2048到3550之间就可以配置一半可程序使用
				...
				choice = input('Please choice(y:confirm/any other keys:cancel):').strip()
				if choice == 'y':
...
def Init_setup_Resin4(Ecology_dir,Resin_dir,JDK_dir,new_install = True):
	...
	if os.path.exists(resin_xml):
		...
		with open(resin_xml) as f:
			strs2 = f.read()
			Installation_jdk_inernal = re.findall(r'javac compiler="internal"',strs2) # 2022年4月2日发布的Resin4方式改为internal
			if len(Installation_jdk_inernal)>0:
				pass
			else:
				Installation_jdk_list = re.findall(r'javac compiler="(.+?)"',strs2)
				if len(Installation_jdk_list)>0:
...
def open_ports(ports):
    os_info = os.popen("cat /etc/os-release").read().lower()
    ...
    if 'ubuntu' in os_info:
        ...
        if int(os_version) <= 6:  # Centos6
            ...
            (status, output) = subprocess.getstatusoutput('firewall-cmd --state')
            if status == 0:
                print('Firewall running...')
                for p in ports:
                    print(('Opening the port: %s' % p))
...
def main():
	...
	print("""\033[0;35;40m\t
 	\033[0;35;40m _____          _   \033[0m                 
	\033[0;35;40m| ____|___ ___ | | ___   __ _ _   _  \033[0m   [1]This software is written in Python, and there is no infringement of third-party software.
	\033[0;35;40m|  _| / __/ _ \| |/ _ \ / _` | | | | \033[0m   [2]Function description:
	\033[0;35;40m| |__| (_| (_) | | (_) | (_| | |_| |  \033[0m   1: Make sure the "unzip" and "tar" commands are available on the server.
	\033[0;35;40m|_____\___\___/|_|\___/ \__, |\__, |  \033[0m   2: The installation file has been extracted and 												
	\033[0;35;40m                        |___/ |___/   \033[0m      can be configured according to the specified directory.
